...
const CONTEXT_DB_COMMANDS = {
    Create_Table: {
        Base_Nodes: `CREATE TABLE IF NOT EXISTS base_nodes ( node_id uuid PRIMARY KEY, name VARCHAR(50), secret VARCHAR(50), org_name VARCHAR(50), csr_hosts VARCHAR(50), host VARCHAR(50), port INT NOT NULL, type INT NOT NULL);`,
        ...
        Peer_Nodes: `CREATE TABLE IF NOT EXISTS peer_nodes ( peer_id uuid PRIMARY KEY, node_id uuid NOT NULL, external_ip VARCHAR(50), internal_ip VARCHAR(50), couchdb_host VARCHAR(50), couchdb_port VARCHAR(50), couchdb_username VARCHAR(50), couchdb_password VARCHAR(50),FOREIGN KEY (node_id) REFERENCES base_nodes (node_id));`
    },
    Fetch_Data: {
        ...
        Peer_Node: `SELECT * FROM base_nodes INNER JOIN peer_nodes ON base_nodes.node_id = peer_nodes.node_id`
    },
    Insert_Data: {
        Base_Node: (node, node_id) => `INSERT INTO base_nodes(node_id, name, secret, org_name, csr_hosts, host, port, type) VALUES (${`\'${node_id}\'`}, '${node.name}', '${node.secret}', '${node.orgName}', ${node.csrHosts}, '${node.host}', ${node.port}, ${node.type});`,
        ...
        Peer_Node: (node, node_id, type_id) => `INSERT INTO peer_nodes(peer_id, node_id, external_ip, internal_ip, couchdb_host, couchdb_port, couchdb_username, couchdb_password) VALUES (${`\'${type_id}\'`}, ${`\'${node_id}\'`}, '${node.externalIp}', '${node.internalIp}', '${node.couchDbConfig.host}', '${node.couchDbConfig.port}', '${node.couchDbConfig.username}', '${node.couchDbConfig.password}');`
    }
}
...
async function _createTables(db) {
    ...
    try {
        ...
        await client.query(CONTEXT_DB_COMMANDS.Create_Table.Peer_Nodes);
    } catch (e) {
...
function _prepareQueries(node) {
    ...
    switch (node.constructor) {
        ...
        case PeerNode:
        case RemotePeerNode:
            nodeQuery = CONTEXT_DB_COMMANDS.Insert_Data.Peer_Node(node, node_id, type_id);
            break;
    }
